...
- `GET /servers`
  ...
  - 响应字段：`name`、`address`、`online`、`map`、`mapTier`、`players`、`maxPlayers`、`note`、`steamAppId`（可选，0/省略=不指定游戏）、`onlinePlayers[]`（`name`、`auth`、`durationSeconds`、`durationDisplay`）。
...
- 服务器页（`/servers`）：`ServerInfoPanel` 在 `loading` 时于同一面板壳内渲染单条服务器占位（地图框 + 玩家列表 + 加入按钮）；顶栏为当前地图名 + Tier + 在线/离线；`GET /servers` 每 30s 轮询。「加入」：未配置 `steamAppId`（或为 0）时用 `steam://connect/host:port`；配置了 AppID 时用 `steam://run/{id}//+connect host:port`（仅触发一次，不做延迟补连）。
...
- `SurfWeb:Servers[]`：公开默认值使用 `127.0.0.1:27015` 作为模板占位；真实服务器放入本地配置。字段包含 `Name`、`Address`（`connect host:port` 或 `host:port`）、可选 `Host`/`Port` 覆盖（`Host` 只写主机名或 IP，**不要**带端口；域名会在 A2S 查询前 DNS 解析）、`MaxPlayers`、可选 `SteamAppId`（不填或 `0` → 前端 `steam://connect`；如 `240` → `steam://run/240//+connect`）。
