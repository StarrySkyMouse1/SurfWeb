...
namespace SurfWeb.Application.Web.Middleware;

/// <summary>
/// 读 API 最小响应时间：耗时不足配置值则补齐等待；超过则立即返回。
/// </summary>
    ...
    IOptions<SurfWebOptions> options)
{
    ...
    internal static TimeSpan ResolveDelay(double seconds) =>
        seconds <= 0 ? TimeSpan.Zero : TimeSpan.FromSeconds(seconds);

    internal static bool AppliesTo(PathString path)
    {
