...
public sealed class ServerQueryService(
    ...
    ServerStatusRefresher refresher) : IServerQueryService
{
    public async Task<IReadOnlyList<ServerStatusDto>> GetStatusesAsync(CancellationToken ct = default)
    {
        ...
        var cacheStale = store.LastUpdatedUtc is null
            || DateTime.UtcNow - store.LastUpdatedUtc.Value > TimeSpan.FromSeconds(refreshSeconds);

        if (cacheStale)
            await refresher.RefreshAsync(ct);

        var snapshotByIndex = store.Snapshot.ToDictionary(s => s.Index);
