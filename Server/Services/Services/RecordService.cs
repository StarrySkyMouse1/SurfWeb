    ...
    IOptions<SurfWebOptions> options) : IRecordService
{
    ...
    {
        var recentRuns = await LoadRecentPlayerRunsAsync(null, ct);
        var recentMainRuns = await LoadRecentPlayerRunsAsync(TrackRankingScope.Main, ct);
        var recentBonusRuns = await LoadRecentPlayerRunsAsync(TrackRankingScope.Bonus, ct);
        var recentStageRuns = await LoadRecentStageRunsAsync(ct);

        if (recentRuns.Count == 0
            && recentMainRuns.Count == 0
            && recentBonusRuns.Count == 0
            && recentStageRuns.Count == 0)
            return new RecentRecordsSnapshot([], [], [], [], [], [], []);

        var maps = recentRuns.Select(pt => pt.Map)
            .Concat(recentMainRuns.Select(pt => pt.Map))
            .Concat(recentBonusRuns.Select(pt => pt.Map))
        ...
        var authIds = recentRuns
            ...
            .Concat(recentMainRuns.Where(pt => pt.Auth != null).Select(pt => pt.Auth!.Value))
            .Concat(recentBonusRuns.Where(pt => pt.Auth != null).Select(pt => pt.Auth!.Value))
        ...
        var mainRuns = PickTopPlayerPbByDate(recentMainRuns);
        var bonusRuns = PickTopPlayerPbByDate(recentBonusRuns);
        ...
        var wrMain = BuildDtos(DedupeFastestPerPlayerMapTrack(recentMainRuns), names, wrByRun, tiers)
            .Where(IsWr)
            .OrderByDescending(d => d.Date)
            .ThenByDescending(d => d.Id)
            .Take(SiteLimits.MaxRecentTotal)
        ...
        var wrBonus = BuildDtos(DedupeFastestPerPlayerMapTrack(recentBonusRuns), names, wrByRun, tiers)
            .Where(IsWr)
            .OrderByDescending(d => d.Date)
            .ThenByDescending(d => d.Id)
            .Take(SiteLimits.MaxRecentTotal)
        ...
        var wrStage = BuildStageDtos(DedupeFastestPerStage(recentStageRuns), names, wrByStage, tiers)
            .Where(IsWr)
            .OrderByDescending(d => d.Date)
            .ThenByDescending(d => d.Id)
            .Take(SiteLimits.MaxRecentTotal)
    ...
    private async Task<List<PlayerTime>> LoadRecentPlayerRunsAsync(
        TrackRankingScope? scope,
        CancellationToken ct)
    {
        var query = playerTimes.Where(pt => pt.Auth != null && pt.Date != null);

        query = scope switch
        {
            TrackRankingScope.Main => query.Where(pt => pt.Track == 0),
            TrackRankingScope.Bonus => query.Where(pt => pt.Track > 0),
            _ => query,
        };
    ...
    private async Task<List<StageTime>> LoadRecentStageRunsAsync(CancellationToken ct) =>
        await stageTimes
